# Solution: Lab 6.7 - Threat Detection - Attack Simulation and Response
...
kubectl exec -n lab-6-7 $ATTACKER -- /bin/sh -c "id && hostname"
```
...
kubectl exec -n lab-6-7 $ATTACKER -- /bin/sh -c "echo 'attack simulation'"
...
kubectl exec -n lab-6-7 attacker-pod -- ps aux > /tmp/attacker-processes.txt 2>/dev/null || true
kubectl exec -n lab-6-7 attacker-pod -- netstat -an > /tmp/attacker-connections.txt 2>/dev/null || true
