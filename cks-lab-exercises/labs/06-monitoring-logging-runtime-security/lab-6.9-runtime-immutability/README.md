# Lab 6.9: Runtime Immutability - Immutable Containers and Read-Only Filesystems
...
kubectl wait --for=condition=Ready pod/immutable-pod -n lab-6-9 --timeout=60s

# Try to write to root filesystem (should FAIL)
kubectl exec -n lab-6-9 immutable-pod -- touch /test-file 2>&1 || echo "Write blocked as expected"

# Try to write to /tmp (should SUCCEED)
kubectl exec -n lab-6-9 immutable-pod -- touch /tmp/test-file && echo "Write to /tmp succeeded"

# Try to write to /var/cache/nginx (should SUCCEED)
kubectl exec -n lab-6-9 immutable-pod -- touch /var/cache/nginx/test && echo "Write to cache succeeded"
```
