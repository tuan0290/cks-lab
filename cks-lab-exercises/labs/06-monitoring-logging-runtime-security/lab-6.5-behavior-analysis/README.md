# Lab 6.5: Container Behavior Analysis with Falco and Audit Logs
...
3. **Task**: Create a Pod named `behavior-test` in namespace `lab-6-5` using image `alpine:3.19` with command `["sleep", "3600"]`.

4. **Task**: Generate some behavior events:
   ```bash
   kubectl exec behavior-test -n lab-6-5 -- sh -c "ls /etc; cat /etc/hostname; id"
   ```

5. **Task**: Analyze Falco logs with `jq`:
   ```bash
   kubectl logs -n falco -l app.kubernetes.io/name=falco --tail=50 | \
     grep "lab-6-5" | head -10
   ```

6. **Task**: Create a ConfigMap named `behavior-analysis-report` in namespace `lab-6-5` documenting:
...
```bash
# Get the test pod
TEST_POD=$(kubectl get pod -n lab-6-5 -l app=behavior-test -o jsonpath='{.items[0].metadata.name}')

# Generate various behaviors
kubectl exec -n lab-6-5 $TEST_POD -- ls /etc/
kubectl exec -n lab-6-5 $TEST_POD -- cat /etc/hostname
kubectl exec -n lab-6-5 $TEST_POD -- wget -q --spider http://example.com 2>/dev/null || true
kubectl exec -n lab-6-5 $TEST_POD -- ps aux
```
