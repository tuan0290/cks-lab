# Lab 6.4: Falco Custom Rules - Privileged Container Detection
...
3. **Task**: Create a privileged Pod named `privileged-test` in namespace `lab-6-4` to trigger the rule:
   ```yaml
   securityContext:
     privileged: true
   ```
...
5. **Task**: Create a ConfigMap named `privileged-detection-results` in namespace `lab-6-4` documenting the Falco alert output.
...
kubectl get pod privileged-test -n lab-6-4 -o jsonpath='{.spec.containers[0].securityContext}'
...
# Clean up
kubectl delete pod priv-test-2 -n lab-6-4 --ignore-not-found=true
```
