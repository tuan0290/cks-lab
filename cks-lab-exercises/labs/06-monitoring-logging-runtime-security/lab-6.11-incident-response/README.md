# Lab 6.11: Kubernetes Incident Response Procedures
...
kubectl logs suspicious-pod -n lab-6-11 --tail=20

# Check if the pod has unusual RBAC permissions
kubectl get rolebindings,clusterrolebindings -n lab-6-11 -o yaml | grep -A5 "suspicious"
```
...
kubectl exec -n lab-6-11 suspicious-pod -- mount >> /tmp/incident-evidence.txt 2>/dev/null || true
