# Lab 6.12: Network Traffic Monitoring and Anomaly Detection
...
kubectl exec -n lab-6-12 $TEST_POD -- wget -qO- --timeout=3 http://1.2.3.4:4444 2>/dev/null || true

# Test DNS resolution (should work)
kubectl exec -n lab-6-12 $TEST_POD -- nslookup kubernetes.default.svc.cluster.local 2>/dev/null || true
```
