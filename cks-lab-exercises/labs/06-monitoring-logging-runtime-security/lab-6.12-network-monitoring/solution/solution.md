# Solution: Lab 6.12 - Network Traffic Monitoring and Anomaly Detection
...
```bash
TEST_POD=$(kubectl get pod -n lab-6-12 -l app=network-test -o jsonpath='{.items[0].metadata.name}')

# Test legitimate internal connection (should NOT trigger alert)
kubectl exec -n lab-6-12 $TEST_POD -- wget -qO- --timeout=3 http://internal-svc 2>/dev/null || true

# Test suspicious port connection (SHOULD trigger alert)
kubectl exec -n lab-6-12 $TEST_POD -- nc -z -w1 1.2.3.4 4444 2>/dev/null || true
...
spec:
  ...
  - to:
    - namespaceSelector:
        matchLabels:
          kubernetes.io/metadata.name: lab-6-12
    ports:
    - port: 80
      protocol: TCP
    - port: 8080
...
kubectl exec -n lab-6-12 $TEST_POD -- wget -qO- --timeout=3 http://1.2.3.4 2>&1 || echo "Blocked as expected"

# Internal traffic should still work
kubectl exec -n lab-6-12 $TEST_POD -- wget -qO- --timeout=3 http://internal-svc 2>/dev/null && echo "Internal traffic works"
```
