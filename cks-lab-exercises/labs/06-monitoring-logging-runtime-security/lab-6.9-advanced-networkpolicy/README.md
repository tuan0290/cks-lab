# Lab 6.9: Advanced NetworkPolicy - Multi-Tier Application Isolation
...
1. **Task**: Create namespace `lab-6-9` and deploy 3 pods:
...
2. **Task**: Create a NetworkPolicy named `default-deny-all` in namespace `lab-6-9` that denies **all** ingress and egress for all pods (`podSelector: {}`, `policyTypes: [Ingress, Egress]`).

3. **Task**: Create a NetworkPolicy named `allow-frontend-ingress` in namespace `lab-6-9` that allows ingress to `tier=frontend` pods on TCP port 80 from any source.

4. **Task**: Create a NetworkPolicy named `allow-frontend-to-backend` in namespace `lab-6-9` that allows:
   - Egress from `tier=frontend` to `tier=backend` on TCP port 8080
   - Ingress to `tier=backend` from `tier=frontend` on TCP port 8080

5. **Task**: Create a NetworkPolicy named `allow-backend-to-database` in namespace `lab-6-9` that allows:
   - Egress from `tier=backend` to `tier=database` on TCP port 5432
   - Ingress to `tier=database` from `tier=backend` on TCP port 5432

6. **Task**: Create a NetworkPolicy named `allow-dns` in namespace `lab-6-9` that allows egress to `kube-system` namespace on UDP/TCP port 53 for all pods.
...
This creates the `lab-6-9` namespace with frontend, backend, and database pods.

### Step 2: Verify initial connectivity (before policies)

```bash
# Get pod names
FRONTEND=$(kubectl get pod -n lab-6-9 -l tier=frontend -o jsonpath='{.items[0].metadata.name}')
BACKEND=$(kubectl get pod -n lab-6-9 -l tier=backend -o jsonpath='{.items[0].metadata.name}')
DB=$(kubectl get pod -n lab-6-9 -l tier=database -o jsonpath='{.items[0].metadata.name}')

# Test connectivity (should work before policies)
kubectl exec -n lab-6-9 $FRONTEND -- wget -qO- --timeout=3 http://backend-svc:8080/health 2>/dev/null && echo "CONNECTED" || echo "FAILED"
```
