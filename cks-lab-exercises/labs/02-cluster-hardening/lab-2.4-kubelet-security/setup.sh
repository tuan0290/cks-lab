...
echo -e "${GREEN}=== Lab Setup: Kubelet Security Configuration ===${NC}"
echo ""
...
create_resources() {
    ...
    # Create namespace
    kubectl create namespace lab-2-4 --dry-run=client -o yaml | kubectl apply -f - > /dev/null 2>&1
    echo -e "${GREEN}✓ Namespace lab-2-4 ready${NC}"
...
echo "Resources created in namespace: lab-2-4"
kubectl get all -n lab-2-4 2>/dev/null || true
